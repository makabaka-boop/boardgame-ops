...
def _enrich_risk_indicators(metrics_df, results_df, feedbacks_df,
...
def _determine_risk_level(row, participation_threshold, duration_risk_ratio,
                          score_risk_ratio):
    flags = 0
...
def _suggested_action(row, participation_threshold, duration_threshold,
...
def _review_conclusion(row, participation_threshold, duration_threshold,
                       score_threshold, duration_risk_ratio, score_risk_ratio):
    issues = []
    if row['参与率'] < participation_threshold:
        issues.append(
...
def get_high_risk_activities(signups_df, results_df, feedbacks_df,
    ...
    merged = _enrich_risk_indicators(metrics_df, results_df, feedbacks_df,
                                     duration_threshold, score_threshold)
...
def generate_review_suggestions(signups_df, results_df, feedbacks_df,
    ...
    merged['风险等级'] = merged.apply(
        lambda row: _determine_risk_level(row, participation_threshold,
                                          duration_risk_ratio, score_risk_ratio),
        axis=1
    )
    merged['建议动作'] = merged.apply(
        lambda row: _suggested_action(row, participation_threshold, duration_threshold,
                                      score_threshold, duration_risk_ratio, score_risk_ratio),
        axis=1
    )
    merged['复盘结论'] = merged.apply(
        lambda row: _review_conclusion(row, participation_threshold, duration_threshold,
                                       score_threshold, duration_risk_ratio, score_risk_ratio),
        axis=1
    )

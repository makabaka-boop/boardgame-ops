...
DATA_DIR = 'data'
TOTAL_REGISTERED_PLAYERS = 50

DEFAULT_PARTICIPATION_THRESHOLD = 0.30
DEFAULT_DURATION_THRESHOLD = 45
...
def calculate_activity_metrics(signups_df, results_df, feedbacks_df,
                                total_players=TOTAL_REGISTERED_PLAYERS):
    ...
    merged['参与率'] = merged['参与人数'] / total_players
    ...
    merged['平均评分'] = merged['平均评分'].fillna(3.5)
...
def _enrich_risk_metrics(metrics_df, results_df, feedbacks_df,
...
def _check_risk_flags(row, participation_threshold, duration_threshold,
                      score_threshold, duration_risk_ratio, score_risk_ratio):
    flags = []
...
def determine_risk_level(flags_count):
    if flags_count == 0:
        return '低'
    if flags_count == 1:
        return '中'
...
def get_suggested_actions(flags):
    action_map = {
        'participation': '加强宣传推广，优化报名渠道',
        'duration': '调整活动节奏，增加游戏深度',
        'score': '收集负面反馈细节，改善体验设计',
    }
    actions = [action_map[f] for f in flags]
    if not actions:
        actions.append('维持现有运营策略')
    return '；'.join(actions)


def get_review_conclusion(row, flags, participation_threshold, duration_threshold,
                           score_threshold, duration_risk_ratio, score_risk_ratio):
    issues = []
    if 'participation' in flags:
        issues.append(
    ...
    if 'duration' in flags:
        issues.append(
            f"流程节奏偏短（{int(row['短时长对局数'])}/{int(row['对局总数'])}局"
            f"低于{duration_threshold}分钟，占比{row['短时长占比']*100:.0f}%）"
        )
    if 'score' in flags:
        issues.append(
            f"体验反馈偏低（{int(row['低评分反馈数'])}/{int(row['反馈人数'])}人"
...
def _build_risk_type_description(row, participation_threshold, duration_threshold,
                                  score_threshold, flags):
    parts = []
    if 'participation' in flags:
        parts.append(f'参与率低({row["参与率"]*100:.0f}%)')
    if 'duration' in flags:
        parts.append(f'时长短({int(row["短时长对局数"])}/{int(row["对局总数"])}局<{duration_threshold}分)')
    if 'score' in flags:
        parts.append(f'评分低({int(row["低评分反馈数"])}/{int(row["反馈人数"])}人<{score_threshold}分)')
    return '; '.join(parts)
...
def get_high_risk_activities(signups_df, results_df, feedbacks_df,
    ...
    merged = _enrich_risk_metrics(metrics_df, results_df, feedbacks_df,
                                   duration_threshold, score_threshold)
    ...
    high_risk = merged[risk_mask].copy()

    high_risk['风险类型'] = high_risk.apply(
        lambda x: _build_risk_type_description(
            x, participation_threshold, duration_threshold, score_threshold,
            _check_risk_flags(x, participation_threshold, duration_threshold,
                              score_threshold, duration_risk_ratio, score_risk_ratio)
        ),
        axis=1
    )
...
def get_short_duration_games(results_df, duration_threshold=DEFAULT_DURATION_THRESHOLD):
    return results_df[results_df['时长'] < duration_threshold].sort_values('时长')


def get_low_score_feedbacks(feedbacks_df, score_threshold=DEFAULT_SCORE_THRESHOLD):
...
def generate_review_suggestions(signups_df, results_df, feedbacks_df,
    ...
    def _apply_risk(row):
        flags = _check_risk_flags(row, participation_threshold, duration_threshold,
                                   score_threshold, duration_risk_ratio, score_risk_ratio)
        row['风险等级'] = determine_risk_level(len(flags))
        row['建议动作'] = get_suggested_actions(flags)
        row['复盘结论'] = get_review_conclusion(
            row, flags, participation_threshold, duration_threshold,
            score_threshold, duration_risk_ratio, score_risk_ratio
        )
        return row

    merged = merged.apply(_apply_risk, axis=1)

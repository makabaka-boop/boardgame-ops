...
def render_overview_metrics(metrics_df, signups_df, high_risk_df):
...
def render_duration_tab(results_df, duration_threshold, short_games_df):
    ...
    with col_dur1:
        ...
        results_plot_df['时长状态'] = results_plot_df['时长'].apply(
...
def render_feedback_tab(feedbacks_df, score_threshold, low_score_df):
    ...
    feedbacks_plot_df = feedbacks_df.copy()
    feedbacks_plot_df['评分状态'] = feedbacks_plot_df['评分'].apply(
...
def render_risk_tab(metrics_df, high_risk_df, results_df, feedbacks_df,
    ...
    with col_th1:
        ...
        results_risk_df = results_df.copy()
        results_risk_df['高风险'] = results_risk_df['时长'] < duration_threshold
    ...
    with col_th2:
        ...
        feedbacks_risk_df = feedbacks_df.copy()
        feedbacks_risk_df['高风险'] = feedbacks_risk_df['评分'] < score_threshold
...
def render_review_tab(signups_df, results_df, feedbacks_df,
                      participation_threshold, duration_threshold, score_threshold,
                      duration_risk_ratio, score_risk_ratio):
...
def render_raw_data_preview(signups_df, results_df, feedbacks_df):
    st.markdown("---")
    st.subheader("📁 原始数据预览")
...
def main():
    ...
    render_overview_metrics(metrics_df, signups_df, high_risk_df)
    ...
    with tab3:
        render_duration_tab(results_df, duration_threshold, short_games_df)
    with tab4:
        render_feedback_tab(feedbacks_df, score_threshold, low_score_df)
    with tab5:
        render_risk_tab(metrics_df, high_risk_df, results_df, feedbacks_df,
                        participation_threshold, duration_threshold, score_threshold,
                        duration_risk_ratio, score_risk_ratio)
    with tab6:
        render_review_tab(signups_df, results_df, feedbacks_df,
                          participation_threshold, duration_threshold, score_threshold,
                          duration_risk_ratio, score_risk_ratio)

    render_raw_data_preview(signups_df, results_df, feedbacks_df)

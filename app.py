...
def render_sidebar():
    ...
    return (participation_threshold, duration_threshold, score_threshold,
            duration_risk_ratio, score_risk_ratio)
...
def render_tab_popularity(signups_df):
    st.subheader("活动热度排行")
    activity_popularity = get_activity_popularity(signups_df)

    fig_popularity = px.bar(
        x=activity_popularity.values, y=activity_popularity.index,
        orientation='h', title="各活动参与人数排行",
        labels={'x': '参与人数', 'y': '活动名称'},
        color=activity_popularity.values, color_continuous_scale='Viridis'
    )
    ...
    fig_trend = px.line(
        daily_participation, x='日期', y='玩家',
        title="每日活跃玩家数趋势", labels={'玩家': '活跃玩家数'}
    )
    st.plotly_chart(fig_trend, use_container_width=True)


def render_tab_retention(signups_df):
    st.subheader("玩家留存分析")
    ...
    with col_ret1:
        fig_retention = px.histogram(
            player_retention, x='参与次数',
            title="玩家参与次数分布", nbins=20,
    ...
    with col_ret2:
        ...
        fig_pie = px.pie(
            values=retention_summary.values, names=retention_summary.index,
            title="玩家活跃度分布", hole=0.4
        )
...
def render_tab_duration(results_df, duration_threshold, short_games_df, duration_data):
    st.subheader("对局时长分布")
    ...
    with col_dur1:
        ...
        fig_duration.add_vline(
            x=duration_threshold, line_dash="dash", line_color="red",
    ...
    with col_dur2:
        fig_box = px.box(
            results_df, x='活动名', y='时长',
            title="各活动对局时长箱线图", labels={'时长': '时长(分钟)'}
        )
        fig_box.add_hline(
            y=duration_threshold, line_dash="dash", line_color="red",
            annotation_text=f"预警线: {duration_threshold}分钟"
        )
    ...
    with col_m3:
        st.metric("低于阈值对局数", f"{len(short_games_df)} 局 ({len(short_games_df)/len(results_df)*100:.1f}%)")


def render_tab_feedback(feedbacks_df, score_threshold, low_score_df):
    st.subheader("反馈问题排行")
    ...
    with col_fb1:
        fig_feedback = px.bar(
            x=feedback_rank.values, y=feedback_rank.index,
            orientation='h', title="反馈标签分布",
            labels={'x': '出现次数', 'y': '反馈标签'},
            color=feedback_rank.values, color_continuous_scale='RdYlGn_r'
        )
    ...
    with col_fb2:
        ...
        fig_score = px.bar(
            x=avg_score_by_tag.values, y=avg_score_by_tag.index,
            orientation='h', title="各反馈标签平均评分",
            labels={'x': '平均评分', 'y': '反馈标签'},
            color=avg_score_by_tag.values, color_continuous_scale='RdYlGn',
            range_color=[2, 5]
        )
    ...
    fig_score_scatter.add_hline(
        y=score_threshold, line_dash="dash", line_color="red",
...
def render_tab_risk_warning(high_risk_df, metrics_df, results_df, feedbacks_df,
    ...
    fig_participation.add_trace(go.Scatter(
        x=metrics_df['日期'], y=metrics_df['参与率'] * 100,
        mode='markers', name='活动参与率',
        marker=dict(
    ...
    fig_participation.add_hline(
        y=participation_threshold * 100, line_dash="dash", line_color="red",
        annotation_text=f"预警线: {participation_threshold*100:.0f}%"
    )
    fig_participation.update_layout(
        title="参与率散点图与预警阈值", yaxis_title="参与率 (%)", height=400
    )
    ...
    with col_th1:
        ...
        fig_duration_real = px.scatter(
            results_df, x='日期', y='时长', color='高风险',
    ...
    with col_th2:
        ...
        fig_score_real = px.scatter(
            feedbacks_df, x='日期', y='评分', color='高风险',
            hover_data=['活动名', '玩家', '反馈标签'],
...
def render_tab_review(signups_df, results_df, feedbacks_df,
                      participation_threshold, duration_threshold, score_threshold,
                      duration_risk_ratio, score_risk_ratio):
    ...
    if len(filtered_df) == 0:
        ...
        st.markdown(f"**{risk_emoji.get(selected_row['风险等级'], '')} 风险等级：{selected_row['风险等级']}**")
...
def render_raw_data(signups_df, results_df, feedbacks_df):
    st.markdown("---")
    st.subheader("📁 原始数据预览")
    ...
    if data_option == "报名记录":
        ...
        st.dataframe(results_df.drop(columns=['时长状态', '高风险'], errors='ignore'), use_container_width=True)
    else:
        st.dataframe(feedbacks_df.drop(columns=['评分状态', '高风险'], errors='ignore'), use_container_width=True)
...
def main():
    ...
    (participation_threshold, duration_threshold, score_threshold,
     duration_risk_ratio, score_risk_ratio) = render_sidebar()
    ...
    short_games_df = get_short_duration_games(results_df, duration_threshold)
    low_score_df = get_low_score_feedbacks(feedbacks_df, score_threshold)
    ...
    with tab1:
        render_tab_popularity(signups_df)
    with tab2:
        render_tab_retention(signups_df)
    with tab3:
        render_tab_duration(results_df, duration_threshold, short_games_df, duration_data)
    with tab4:
        render_tab_feedback(feedbacks_df, score_threshold, low_score_df)
    with tab5:
        render_tab_risk_warning(
            high_risk_df, metrics_df, results_df, feedbacks_df,
            participation_threshold, duration_threshold, score_threshold,
            duration_risk_ratio, score_risk_ratio
        )
    with tab6:
    ...
    render_raw_data(signups_df, results_df, feedbacks_df)
